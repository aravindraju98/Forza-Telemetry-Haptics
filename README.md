...
Independent Windows app that turns Forza Horizon 6 **Data Out** UDP into layered **XInput** rumble.
...
You do not need Visual Studio or the .NET SDK.
...
Each layer has **Off | On**. Hover **?** for a short tip. Drag the curve like a fan graph. A global **MAX RUMBLE** cap keeps the pad from locking at full strength.

## Requirements

- Windows 10 or 11
- Forza Horizon 6 with Data Out on (other Horizon titles that send the same 324-byte packet also work)
- Any Windows **XInput** pad with rumble
...
Plug in any pad Windows treats as an Xbox controller. Use **XInput / Windows** mode if the pad has a switch. Maker apps are not required.

Use **Left**, **Right**, **Both**, and **Stop** to confirm rumble before you drive.

If two pads are plugged in, the app uses the first XInput slot (index 0). Set `ControllerIndex` in `config.json` to pick another.
...
Only needed if you want to change the code. Regular use is the [release zip](https://github.com/aravindraju98/Forza-Telemetry-Haptics/releases).
...
The .NET 8 SDK is required for this section.
